...
u_char *decode_jpeg (const char *filename, info_t *jpeg_inf)
{
...
u32_t * rgb24to32(u8_t *buf24, info_t jpeg_inf)
{
	int w = jpeg_inf.w;
	int h = jpeg_inf.h;
...
u16_t * rgb24to16(u8_t *buf24, info_t jpeg_inf)
{
	u16_t *buf16 = NULL;
...
u8_t * scale24(u8_t *buf24, info_t new_inf, info_t jpeg_inf)
{
	int i,j;
	int dtw,dth;
