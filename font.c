...
int display_font(const char *code,int *len,int x,int y,color_t color)
{
	unsigned int ucode;
	FT_UInt glyph_index;
	int row, pixel;
...
/* display string */
int display_string (const char * buf, int x, int y, color_t color)
{
	int len;

	while (buf[0] != '\0') 	{	
		if (display_font(buf,&len,x,y,color)) return 1;
		buf += len;
		x += psize;
	}
